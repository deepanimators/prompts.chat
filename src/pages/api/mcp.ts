...
function createServer(options: ServerOptions = {}) {
                  ...
                  content: [
                    {
                      ...
                      text: JSON.stringify({
                          ...
                          link: `https://tucprompt.vercel.app/prompts/${prompt.id}_${getPromptName(prompt)}`,
                        }),
                    },
                  ],
                };
  ...
  server.registerTool(
    "save_prompt",
    {
      title: "Save Prompt",
      description:
        "Save a new prompt to your tucprompt.vercel.app account. Requires API key authentication. Prompts are private by default unless configured otherwise in settings.",
      inputSchema: {
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                  ...
                  prompt: {
                    ...
                    tags: prompt.tags.map((t) => t.tag.name),
                    category: prompt.category?.name || null,
                    link: prompt.isPrivate ? null : `https://tucprompt.vercel.app/prompts/${prompt.id}_${getPromptName(prompt)}`,
                  },
                }),
            },
          ],
  ...
  server.registerTool(
    "save_skill",
    {
      title: "Save Skill",
      description:
        "Save a new Agent Skill to your tucprompt.vercel.app account. Skills are multi-file prompts that can include SKILL.md (required), reference docs, scripts, and configuration files. Requires API key authentication. If the file contents are too long, first save the SKILL.md only, with no other files. Then call add_file_to_skill tool per file.",
      inputSchema: {
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                  ...
                  skill: {
                    ...
                    link: skill.isPrivate ? null : `https://tucprompt.vercel.app/prompts/${skill.id}_${getPromptName(skill)}`,
                  },
                }),
            },
          ],
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                  ...
                  skillId,
                  files: files.map(f => f.filename),
                  link: `https://tucprompt.vercel.app/prompts/${skill.id}_${getPromptName(skill)}`,
                }),
            },
          ],
        };
      ...
      try {
        ...
        const results = skills.map((s) => {
          ...
          return {
            ...
            link: `https://tucprompt.vercel.app/prompts/${s.id}_${getPromptName(s)}`,
          };
        });
